...
class VoiceState:
    def __init__(self, bot: commands.Bot, ctx: commands.Context):
        ...
        self.songnames = [] # save search terms last 20 songs played
        self.radio = False
    ...
    async def song_regex(self,song):
        '''Returns list of search terms. Takes video title as string'''
        if not song:
            return
        if re.search(r"(.+)[-:](.*)\s(?=[\[(])",song):
            match = re.search(r"(.+)[-:](.*)\s(?=[\[(])",song)
        elif re.search(r"(.+)by(.*)\s(?=[\[(])",song):
            match = re.search(r"(.+)by(.*)\s(?=[\[(])",song)
        elif re.search(r"(.+)[-:](.*)",song):
            match = re.search(r"(.+)[-:](.*)",song)
        elif re.search(r"(.+)by(.*)",song):
            match = re.search(r"(.+)by(.*)",song)
        else:
            match = re.search(r"(.+)",song)
        search_terms = match.groups()
    ...
    async def update_songnames(self,song):
        ...
        if len(self.songnames) < 20:
            match = await self.song_regex(song)
            self.songnames.append(match)
        else:
            self.songnames = self.songnames[1:]
            match = await self.song_regex(song)
            self.songnames.append(match)

    async def get_rec(self,url, repeated = False):
        '''Takes last video played and returns Song Object of next video in generated mix'''

        if self.radio and self.artists and not repeated:
            maxi = len(self.artists)
            rand = random.randint(0,maxi)
            if rand != maxi:
                artist = self.artists[rand]
                source = await YTDLSource.create_source(self._ctx, artist, loop=self.bot.loop)
                url = source.url
        ...
        async with self._ctx.typing():
            try:
                ...
                while check:
                    print("Repeated!")
                    song = await self.get_rec(song.source.url,repeated = True)
...
class Music(commands.Cog):
    ...
    async def _radio(self, ctx: commands.Context, *, lst: str = ''):
        ...
        if lst:
            ...
            commands = {
                    'add': ctx.voice_state._radioadd,
                    'remove': ctx.voice_state._radioremove,
                    'clear': ctx.voice_state._radioclear,
                    'play': ctx.voice_state._radioplay
                }
            ...
            if newlst[0][0] == '-':
                ...
                ctx.voice_state.artists = lst.split(',')

        page = 1
        items_per_page = 10
        pages = math.ceil(len(ctx.voice_state.artists) / items_per_page)

        start = (page - 1) * items_per_page
        end = start + items_per_page

        queue = ''
        for i, artist in enumerate(ctx.voice_state.artists[start:end], start=start):
            queue += '`{0}.` [**{1}**]\n'.format(i + 1, artist)

        embed = (discord.Embed(description='**{} Artists:**\n\n{}'.format(len(ctx.voice_state.artists), queue))
                 .set_footer(text='Viewing page {}/{}'.format(page, pages)))
        await ctx.send(embed=embed)
